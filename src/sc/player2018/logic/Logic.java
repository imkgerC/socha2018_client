      ...
      for (Action action : move.actions) {
        if (action instanceof Advance) {
          ...
          if (advance.getDistance() + index == Constants.NUM_FIELDS - 1) {
            ...
            ratedMoves.add(new RatedMove(move,Integer.MAX_VALUE));

          } else if (gameState.getBoard().getTypeAt(advance.getDistance() + index) == FieldType.SALAD) {
            // Zug auf Salatfeld
            saladMoves.add(move);
            ratedMoves.add(new RatedMove(move,4));
          } else {
            // Ziehe Vorwärts, wenn möglich
            ratedMoves.add(new RatedMove(move,0));
          }
          ...
          if (index > 56 /*letztes Salatfeld*/ && currentPlayer.getSalads() > 0) {
            ...
            selectedMoves.add(move);
            ratedMoves.add(new RatedMove(move,3));
            ...
            selectedMoves.add(move);
            ratedMoves.add(new RatedMove(move,-1));
          }
        } else {
